...
> It is written as a post-mortem and as a reference for anyone learning systematic trading or competition-style quant research.
...
Hi, we are Team RAJ. We are engineering students at Monash, and we are interested in quantitative trading.
...
Here is the setup the competition uses:

- **Universe:** 50 synthetic stocks (0-49), daily prices (about 1500 days).
...
- **Costs and limits:** 5 bps commission on dollars traded, and a +/- $10,000 notional cap per stock (integer shares, clipped if exceeded).
...
Before developing specific trading strategies, we first assessed which approaches were compatible with the data resolution, execution model, and constraints of the competition. Several common strategy classes were evaluated and explicitly excluded or de-emphasised.
...
### De-emphasised: Machine Learning
Machine learning models were explored, but the relatively small sample size (about 1500 daily observations) and low signal-to-noise ratio made reliable generalisation difficult.  
...
To assess whether this was viable, we first examined the cross-asset correlation structure.
...
Given this, we decided not to proceed with deeper analysis such as formal cointegration testing. The data did not provide a strong enough foundation to justify further research into pairs trading, so this approach was excluded from the final algorithm.
...
To test whether directional moves tend to persist, we examined the autocorrelation of daily market returns.
...
### But per-stock momentum is weak and inconsistent
...
- Compared with the market's lag-1 ACF (about **0.4**), even the best single-name momentum is substantially weaker.

For contrast, the bottom-ACF stocks show that negative autocorrelation exists, but it also is not clean or persistent:
...
- Practically, this pushed momentum usage toward **market regime or exposure filters**, rather than a rank-stocks-by-momentum and buy-winners approach.
...
- If the AR(1) coefficient (phi) is **positive**, the stock tends to continue in the same direction (momentum).
- If phi is **negative**, the stock tends to reverse direction (mean reversion).
- If phi is **close to zero**, yesterday's move has little to no predictive power.
...
First we looked at the distribution of rolling volatility across all stocks and days.
...
Next we checked how volatility changes **over time** (market-wide) and **between stocks**.
...
To see how volatility evolves, we looked at a **100-day rolling volatility** of the equal-weight index and marked simple low- and high-volatility bands.
...
<p align="center">
  <em>Figure 13. Rolling 100-day annualised volatility for the top 2 most volatile versus top 2 least volatile stocks.</em>
</p>
...
**Implication:** any strategy that sizes positions uniformly across names is implicitly taking much larger risk in the high-vol stocks. This strongly supports **volatility scaling or risk targeting per instrument** (and explains why naive equal-size signals often get dominated by a small subset of names).

---

### 1.4.5 Short-term autocorrelation: momentum versus mean reversion

Volatility alone does not tell us the direction of returns. To see whether short-term moves tend to **continue** (momentum) or **reverse** (mean reversion), we looked at **rolling autocorrelation** of the market index at different lags.

For each lag, we computed the autocorrelation over a rolling 100-day window:
...
### 1.4.6 Lead-lag structure (market versus stocks)

A natural follow-up question is whether the market moves first and individual stocks respond later (or vice versa). To test this, we computed cross-correlations between the equal-weight market return at time *t* and each stock return at time *(t + lag)*.
...
<p align="center">
  <em>Figure 18. Lead-lag heatmap: correlation between Market(t) and Stock(t + lag). The strongest structure is concentrated at lag 0.</em>
</p>
...
<p align="center">
  <em>Figure 19. Average lead-lag profile (market versus universe). The peak is at lag 0; correlations decay quickly as |lag| increases.</em>
</p>
...
- Correlation decays quickly beyond 1 to 2 days in either direction.

**Conclusion:** lead-lag effects are not a strong, broad edge in this universe. The market and stocks mostly move together contemporaneously, which is more useful for risk or exposure control than for clean prediction.
...
1. **Momentum is mostly market-wide, not per stock.**  
   The market index has strong lag-1 autocorrelation (about 0.4), but even the top 2 per-stock ACF names have meaningfully lower and unstable autocorrelation. This implies momentum is more suitable as a **market exposure or regime filter** than as a single-stock selection signal.

2. **Volatility is very much per stock.**  
   Some stocks are persistently about 3 to 4 times more volatile than others (Figure 13). Risk is dominated by which names you trade and how you size them, so per-stock volatility scaling is essential.
...
4. **Short-term momentum is strongest at very short lags.**  
   The rolling autocorrelation plots show clear positive dependence at 1 to 2 day lags. By 5 to 10 days, the signal is much weaker and often indistinguishable from noise.
...
- If that move is big enough (passes a **threshold**) we take exposure:
  - index clearly up -> go **long**
  - index clearly down -> go **short**
  - small or noisy move -> stay **flat**
- Size each instrument using **realised volatility**, so risk is not dominated by a few wild stocks.
...
2) equity curve (gross versus net)  
...
<p align="center">
  <em>Figure 20. Stock 38 price + trade signals, equity curve, and position trace.</em>
</p>

How to read it:

- **Green ▲** = enter or flip into **long**
- **Red ▼** = enter or flip into **short**
...
- Gross versus net P&L shows the cost of turnover, but net still rises when the regime is clean.
- The messy periods are obvious: when signals get choppy, entries and exits cluster and costs increase.
...
We started with the standard evaluator most teams used (`eval.py` style). We did not rewrite the evaluator; we extended the same logic to run **walk-forward testing** and basic robustness checks.
...
The limitation is that it is still **one window** (one slice of time).
...
- **Shuffle or shift test:** break time alignment to ensure performance drops when structure is removed.

---

## 2.4 Parameter Selection and Robustness
...
<p align="center">
  <em>Figure 21. Lookback versus threshold sweep (last 1000 days). Green = strong score, red = poor or negative. The best region is a broad plateau around lookback ~7-10 and threshold ~0.0005-0.0015.</em>
</p>

**Key pattern:** there is not one fragile needle-in-a-haystack optimum; there is a **wide plateau** of good parameters.

- Very short lookbacks (about 2-5) are unstable and often negative (too noisy, too much churn).
...
<p align="center">
  <em>Figure 22. Cumulative P&L: shows consistent profits across the full Algothon dataset.</em>
</p>
...
Scores near the top were close enough that small differences in behaviour (or just a few extra good or bad days) could shuffle ranks. When the spread is small, it is hard to claim one team is definitely better than another just from final rank alone.
...
- smarter **turnover control** (fewer unnecessary flips, better use of thresholds or hysteresis),
- better **risk limits** (avoiding taking risk during messy periods, or scaling down in high-vol regimes),
- or finding a small **extra alpha source** on top (even a weak edge can matter if it does not add turnover).
...
then strategies that lean harder into that regime will look amazing. That does not mean they will hold up next time.

That is exactly why we valued walk-forward testing: it is not perfect, but it reduces the chance that our strategy only worked because the last block happened to match it.
...
Those strategies can land anywhere depending on noise. If you are relying on fragile edges, rank becomes much more about variance.

**Our honest takeaway:**  
We think our final placement is a mix of:
- a real edge (market momentum was genuinely present),
- solid risk and cost handling (threshold + vol scaling),
- and some unavoidable variance (because the leaderboard is tight at the top).

If we had another iteration, the first place we would look to improve would be **reducing whipsaw damage** (for example, smoother entry and exit logic or regime persistence) without increasing trading frequency.
...
- strong **market-level** short-horizon momentum (especially 1 to 2 day effects).

### Mean reversion was not an easy profit
We expected at least some clean bounce-back after extreme moves, but the market more often showed **follow-through** rather than snapping back. That does not mean mean reversion never works; it just was not dominant at the daily horizon in this universe.

### Regimes matter more than indicators
The strategy did not fail because momentum stopped existing; it mostly failed in specific periods where the market got choppy and direction flipped often. So a lot of the edge comes from:
- recognising when conditions are good for the signal,
- trading less when the regime is unclear.

### Turnover kills strategies
Many strategies looked decent before costs and then became useless after commission. The best improvements we made were not about finding new signals; they were about reducing pointless trades:
...
- **Not focusing on trading cost control from day one.**  
  We initially treated costs as something to add later. In reality, costs decide what is even feasible.

- **Trying to extract per-stock alpha in a market that did not support it.**  
  The data strongly suggested market-level structure was the main thing. We should have committed to that direction earlier.
...
- **Spend more time on damage control.**  
  Our strategy works well in clean regimes. The main weakness is chop. Next iteration, we would focus on better entry and exit smoothing (without increasing trading frequency).
...
### What did not work
- Cross-asset relative value ideas (pairs or cointegration); the correlation structure was simply too weak.
...
### Future work
If we were to iterate on this bot, we would stay in the same general family (market regime + risk control) and focus on:
