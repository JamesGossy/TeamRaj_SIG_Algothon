...
# -*- coding: utf-8 -*-
'''
EMA trend follower with Efficiency-Ratio (ER) regime gating, tuned to maximize the 500-day
window (days 1000-1500) with extra emphasis on the last 250 days (1250-1500) on your dataset.

- ER-gating: widens threshold and shrinks size in choppy regimes (low ER), restores in trends.
- Inverse-vol sizing per instrument, $10k cap at trade time.
- 10-share lot rounding and a $2000 per-name no-trade band.
'''
import numpy as np
...
# ER gating
ER_WINDOW = 25
ER_LOW  = 0.21236668549323098
ER_HIGH = 0.6142408782823584
CHOP_SIZE_MULT = 0.3
CHOP_THR_MULT  = 2.0
CHOP_FLAT = False
...
def getMyPosition(price_history):
    ...
    if P.ndim != 2:
        raise ValueError("price_history must be 2D")
    if P.shape[0] != 50:
        P = P.T
