...
"""
Adaptive index trend follower.

This strategy examines the geometric-mean index of all 50 instruments
and compares it to an exponential moving average with a 7-day span.
If the index deviates from its EMA by at least 0.15 % (i.e.,
|index/EMA - 1| >= 0.0015), the algorithm takes a directional
position (long or short) across all instruments; otherwise it stays flat.
Position sizes are inversely proportional to recent volatility (5-day window).
"""
...
def getMyPosition(price_history: np.ndarray) -> List[int]:
    """
    Compute the desired position for each instrument based on the index trend.
    """
...
if __name__ == "__main__":
    # Print example output for a dummy price history
    dummy = np.random.rand(50, max(SPAN, VOL_WINDOW) + 2)
    print(getMyPosition(dummy))
